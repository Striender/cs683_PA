...
long long run_with_prefetching(const vector<float>& embedding_table, const vector<int>& input, const vector<int>& offsets) {
    ...
    for (int j = start_idx; j < end_idx; ++j) {
        ...
        if (j + prefetch_distance < end_idx) {
            _mm_prefetch((const char*)&embedding_table[input[j + prefetch_distance] * embedding_dim],_MM_HINT_T1);
        }
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (; p <= embedding_dim - 8; p += 8)
        {
            ...
            { if (j + prefetch_distance < end_idx) {
                ...
                const float *data_ptr = &embedding_table[input[j] * embedding_dim];
                __m256 data = _mm256_loadu_ps(&data_ptr[p]);
                sum = _mm256_add_ps(sum, data);
            }
            _mm256_storeu_ps(&bag_embedding[p], sum);
        }
        for (; p < embedding_dim; ++p)
        {
            float sum = 0.0f;
            for (int j = start_idx; j < end_idx; ++j)
            {
